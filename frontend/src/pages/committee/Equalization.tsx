...
function Equalization(){
    ...
    // Fetch closed cycles for history tab
    useEffect(() => {
    ...
    // Fetch collaborators for selected closed cycle
    useEffect(() => {
            ...
            <div className="w-full bg-[#f1f1f1] min-h-full">
                <div className="bg-white w-full min-h-[15%] p-2 sm:p-4 box-border border border-gray-300">
                    ...
                    <div className="flex gap-4 mb-4">
                        <button className={`px-4 py-2 rounded-md font-medium ${activeTab === 'equalization' ? 'bg-[#08605F] text-white' : 'bg-gray-100 text-gray-700'}`} onClick={() => setActiveTab('equalization')}>Equalização</button>
                        <button className={`px-4 py-2 rounded-md font-medium ${activeTab === 'history' ? 'bg-[#08605F] text-white' : 'bg-gray-100 text-gray-700'}`} onClick={() => setActiveTab('history')}>Histórico</button>
                    </div>
                                ...
                                {historyCollaborators.filter(c => c.name.toLowerCase().includes(historySearch.toLowerCase())).map(collab => (
                                    <div key={collab.id} className="bg-white rounded-lg shadow-md min-w-[260px] sm:min-w-0 p-4 flex flex-col gap-2">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="ml-auto font-bold text-green-700 text-lg">{collab.notaFinal !== undefined ? collab.notaFinal.toFixed(2) : '-'}</div>
                                        </div>
                                        <div className="text-xs text-gray-600 mt-1">
                                            <b>Resumo da IA:</b> <AsyncAISummary userId={collab.id} cycleId={collab.cycleId} />
                                        </div>
                                    </div>
                                ))}
