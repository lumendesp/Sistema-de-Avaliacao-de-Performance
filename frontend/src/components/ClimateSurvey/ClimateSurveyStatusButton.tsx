...
interface ClimateSurveyStatusButtonProps {
  survey: any | null;
}

export default function ClimateSurveyStatusButton({ survey }: ClimateSurveyStatusButtonProps) {
  const navigate = useNavigate();

  const parsedEndDate = survey?.endDate ? new Date(survey.endDate) : null;
  const isActive =
    !!survey &&
    survey.isActive &&
    parsedEndDate &&
  ...
  console.log("📅 survey.endDate:", survey?.endDate);
  console.log("✅ isActive:", isActive);

  const handleClick = () => {
    if (isActive) {
      navigate('/collaborator/climate-survey/form');
    }
  };

  const buttonStyle = isActive
    ? 'bg-[#08605F] text-white hover:bg-opacity-90'
  ...
  return (
    <button
      onClick={handleClick}
      disabled={!isActive}
      className={`w-full flex items-center justify-between p-6 rounded-lg transition-colors duration-200 shadow-sm ${buttonStyle}`}
    >
      <div className="flex items-start gap-3 text-left">
        ...
        <div>
          <p className={`font-semibold ${iconColor}`}>
            {isActive ? 'Pesquisa de clima disponível' : 'Sem pesquisa de clima ativa'}
          </p>
          <p className="text-sm opacity-80">
            {isActive && parsedEndDate
              ? `Termina em ${parsedEndDate.toLocaleDateString('pt-BR')}`
              : 'Você será notificado quando houver uma nova pesquisa.'}
          </p>
        </div>
      </div>
      <IoArrowForward className={`text-2xl ${iconColor}`} />
    </button>
  );
}
