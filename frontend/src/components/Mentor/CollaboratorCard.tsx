...
export default function CollaboratorCard({ collaborator }: Props) {
  const { id, name, role, status, selfScore, managerScore, mentorScore } = collaborator;
  ...
  const handleClick = () => {
    navigate(`/mentor/avaliacao/${id}`); // Corrigido para rota correta
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2 text-sm">
          <span className="text-[#1D1D1DBF]">Nota mentor</span>
          {mentorScore !== undefined && mentorScore !== null ? (
            <span className="bg-gray-200 px-2 py-1 rounded text-sm font-semibold text-gray-800 w-12 text-center">
